...
- **`mode: local-proxy`** allocates a high **main proxy port** (HTTP by default). With **`proxy.tls: dev`**, orchport runs **`openssl`** once, writes ephemeral PEMs under the OS temp dir, and serves **HTTPS** for each entry’s `*.localhost` hostname—no cert files to maintain. Requires **`openssl` on PATH** (macOS/Linux CI typically have it). Browser warnings for self-signed are expected unless you trust the cert.
