...
export default defineConfig({
  ...
  proxy: {
    // omit / "auto": allocator chooses the main proxy port
    // number: require that exact main proxy port and fail if unavailable
    port: "auto",
  },
});
```
...
state dir が書けない sandbox では予約ロックを無効化して続行し、`ORCHPORT_PORT_RESERVATION=disabled` を出す。この場合、同時実行の衝突保護は効かない。
...
| Human CLI 出力                           | 済     | `run` / `env` / `list` / `switch` / `doctor` は人間向けに restrained な色・状態記号・`Next:` ヒントを出す。表は `@visulima/tabular` + header divider の borderless 表示。`env` TTY は `global` と proxy ごとの public URL → local URL section に分け、global env の重複表示を避ける。`env <proxy>` は `run <proxy>` と同じ flat env を表示し、proxy ごとの生成 `PORT` を含む。proxy 未指定の `--json` は nested、piped / `--plain` / `--shell` / `--dotenv` は 1 つの flat env stream。`list` / `switch` は `switchables` の実ルート（incoming URL → local target ← target public URL）を表示。child stdout は script 互換を維持。 |
...
| sandbox state フォールバック             | 済     | state 書き込み失敗時は記録スキップ + `ORCHPORT_VOLATILE_STATE=1` を子に渡す。port 予約ロックが使えない場合は `ORCHPORT_PORT_RESERVATION=disabled` で衝突保護なしを明示する。                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| 配布 / release                           | 進行中 | Changesets で version / `CHANGELOG.md` / npm publish 判断を管理。GitHub Releases の single-binary tarball を npm shim と Nix flake の共通配布物にする。release metadata は GitHub Release の `SHA256SUMS` と `nix/release-hashes.json` として生成する。                                                                                                                                                                                                                                                                                                                                                                            |
